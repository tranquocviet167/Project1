...
if(isset($_SESSION['username']) && isset($_SESSION['level']))
{
  ...
  if(isset($_POST['save']))
  {
    ...
    if ($ngayBatDau <= date('Y-m-d')) {
        echo "<script> alert('Ngày bắt đầu công tác phải lớn hơn hoặc bằng ngày hiện tại');</script>";
    }


    // kiem tra nhan vien co dang trong qua trinh cong tac
    $check = "SELECT * FROM cong_tac WHERE nhanvien_id = '$maNhanVien' AND ngay_ket_thuc > " . date('Y-m-d');
    $resultCheck = mysqli_query($conn, $check);
    if(mysqli_num_rows($resultCheck) != 0)
    {
        while ($row = mysqli_fetch_array($resultCheck)) {
            if ($ngayBatDau >= $row['ngay_bat_dau'] && $ngayBatDau <= $row['ngay_ket_thuc'] || $ngayKetThuc >= $row['ngay_bat_dau'] && $ngayKetThuc <= $row['ngay_ket_thuc'] ) {
                $error['time_cong_tac'] = 'error';
            }
        }

        if (isset($error['time_cong_tac'])) {
            $error['dangCongTac'] = 'error';
            echo "<script>alert('Thời gian trùng với lịch công tác đã xếp');</script>";
        }
    }
                  ...
                  <div class="col-md-12">
                   ...
                   <div class="row">
                       ...
                       <div class="form-group col-md-6">
                           ...
                           <input type="date" class="form-control" id="exampleInputEmail1" name="ngayKetThuc" value="<?php echo date('Y-m-d'); ?>">
