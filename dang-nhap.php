...
if (isset($_SESSION['username']) && isset($_SESSION['level'])) {
	...
	if (isset($_POST['login'])) {
		...
		if (!$error) {
			...
			// check user
			$check = "SELECT id, email, mat_khau, quyen, truy_cap, ma_nv FROM tai_khoan WHERE email = '$email' AND trang_thai <>  0";
			...
			if (!empty($row)) {
				...
				if (mysqli_num_rows($result) == 1) {
					if ($row['mat_khau'] == $password) {
						...
						$acc_nv = "SELECT * FROM nhanvien WHERE ma_nv = '" . $maNv . "'";
						$result_nv = mysqli_query($conn, $acc_nv);
						...
						if ($level == 1) {
							...
							if (isset($row_nv['id']) && !empty($row_nv['id'])) {
				...
				$error['check'] = 'Tài khoản đã bị khóa hoặc không tồn tại';
			}
		}
	}
...
?>
	...
	<head>
		...
		<title>ĐỀ TÀI | QUẢN LÝ NHÂN SỰ</title>
    ...
    background-position: center center; /* Đảm bảo hình nền được căn giữa */
...
.login-box-body {
    background-color: transparent; /* Đảm bảo nền của box đăng nhập là trong suốt */
    background-image: url('../images/nen.jpg');
    background-size: cover;
    background-position: center center;
    border-radius: 5px;
	...
	<body class="hold-transition login-page">
		<div class="login-box">
			<div class="login-logo">
				<a href="index.php"><b>QUẢN LÝ NHÂN SỰ</b></a>
			</div>
			<!-- /.login-logo -->
