...
if (isset($_SESSION['username']) && isset($_SESSION['level'])) {
	...
	if ($_GET['maNhanVien']) {
		$showData = "SELECT ct.id as id, ma_cong_tac, ma_nv, ten_nv, ten_chuc_vu, ngay_bat_dau, ngay_ket_thuc, dia_diem, muc_dich FROM  nhanvien nv, chuc_vu cv, cong_tac ct WHERE nv.chuc_vu_id = cv.id AND nv.id = ct.nhanvien_id AND ct.nhanvien_id =" . $_GET['maNhanVien'] . " ORDER BY ct.ngay_tao DESC";
	}
					...
					<div class="box">
						...
						<div class="box-body">
							<?php if ($row_acc['quyen'] == 1) : ?>
								<div class="row">
									<div class="row " style="margin-top: 20px; margin-bottom: 30px; width: 100%">
										<div class="col-sm-12">
											<form action="danh-sach-cong-tac.php" class="form-inline">
												...
												<input type="hidden" name="p" value="collaborate">
												<input type="hidden" name="a" value="list-collaborate">
									...
									<tbody>
										...
										?>
											<tr>
												...
												$ngayBatDau = $arrS['ngay_bat_dau'];

												if ($ngayHienTai > $ngayBatDau && $ngayHienTai < $ngayHetHan) {
													echo '<td><span class="badge bg-blue"> Đang công tác </span></td>';
												} else if ($ngayBatDau > $ngayHienTai) {
													echo '<td><span class="badge bg-gray"> Sắp công tác </span></td>';
												} else {
