...
if (isset($_SESSION['username']) && isset($_SESSION['level'])) {
  ...
  $NV = "SELECT id, ma_nv, ten_nv FROM nhanvien WHERE NOT EXISTS (SELECT ma_nv FROM tai_khoan WHERE nhanvien.ma_nv = tai_khoan.ma_nv) ";
  ...
  if (isset($_POST['save'])) {
    ...
    if (empty($_POST['ma_nv']))
      $error['ma_nv'] = 'Vui lòng chọn <b> nhân viên </b>';

    if (!empty($maNV)) {
      $sqlNv = "SELECT ma_nv FROM tai_khoan where ma_nv ='" . $maNV . "'";
      $rs_checkNv = mysqli_query($conn, $sqlNv);
      if (mysqli_num_rows($rs_checkNv) > 0)
        $error['checkNv'] = 'Nhân viên đã có tài khoản';
    }
    ...
    if (!empty($_POST['email']) && !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
      $error['email_format'] = 'Vui lòng nhập đúng định dạng <b> email </b>';
    }
    $regex = '/^(0|\+84)(9[0-9]|1[2-9]|8[2-9]|3[2-9]|5[2-9]|7[0|6-9])[0-9]{7}$/';
    if (!empty($phone) && !preg_match($regex, $phone)) {
      $error['phone_format'] = 'Vui lòng nhập đúng định dạng <b> số điện thoại </b>';
    }
    ...
    if (!$error) {
      ...
      if ($image) {
        ...
        // insert record
        $insert = "INSERT INTO tai_khoan(ho, ten, ma_nv, hinh_anh, email, mat_khau, so_dt, quyen, trang_thai, truy_cap, ngay_sua, ngay_tao) 
        VALUES('$lastName', '$firstName', '$maNV', '$nameImage', '$email', '$password', '$phone', $level, $status, $access, '$date_create', '$date_update')";
          ...
          <div class="box box-primary">
            ...
            <form role="form" method="POST" enctype="multipart/form-data">
              <div class="box-body">
                ...
                <div class="form-group">
                  <label for="exampleInputPassword1">Chọn nhân viên: <b style="color: red;">*</b></label>
                  <select class="form-control" name="ma_nv">
                    <option value="">--- Chọn nhân viên ---</option>
                    <?php
                    foreach ($arrNV as $nv) {
                      echo "<option value='" . $nv['ma_nv'] . "'>" . $nv['ma_nv'] . " - " . $nv['ten_nv'] . "</option>";
                    }
                    ?>
                  </select>
                  <spam class="text-danger"><?php echo isset($error['ma_nv']) ? $error['ma_nv'] : '' ?></spam>
                  <spam class="text-danger"><?php echo isset($error['checkNv']) ? $error['checkNv'] : '' ?></spam>
                </div>
                <div class="form-group">
                  ...
                  <spam class="text-danger"><?php echo isset($error['lastName']) ? $error['lastName'] : '' ?></spam>
                </div>
                <div class="form-group">
                  ...
                  <spam class="text-danger"><?php echo isset($error['firstName']) ? $error['firstName'] : '' ?></spam>
                </div>
                <div class="form-group">
                  ...
                  <spam class="text-danger"><?php echo isset($error['email']) ? $error['email'] : '' ?></spam>
                  <spam class="text-danger"><?php echo isset($error['email_format']) ? $error['email_format'] : '' ?></spam>
                  <spam class="text-danger"><?php echo isset($error['checkEmail']) ? $error['checkEmail'] : '' ?></spam>
                </div>
                <div class="form-group">
                  ...
                  <spam class="text-danger"><?php echo isset($error['password']) ? $error['password'] : '' ?></spam>
                </div>
                <div class="form-group">
                  ...
                  <spam class="text-danger"><?php echo isset($error['repass']) ? $error['repass'] : '' ?></spam>
                  <spam class="text-danger"><?php echo isset($error['checkPass']) ? $error['checkPass'] : '' ?></spam>
                </div>
                <div class="form-group">
                  <label for="exampleInputPassword1">Số điện thoại:</label>
                  <input type="text" class="form-control" maxlength="10" id="exampleInputPassword1" placeholder="Nhập số điện thoại" name="phone" value="<?php echo isset($_POST['phone']) ? $_POST['phone'] : ''; ?>">
                  <spam class="text-danger"><?php echo isset($error['phone_format']) ? $error['phone_format'] : '' ?></spam>
                </div>
                <div class="form-group">
