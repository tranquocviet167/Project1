...
if (isset($_SESSION['username']) && isset($_SESSION['level'])) {
	...
	if (isset($_GET['id'])) {
		$id = $_GET['id'];
		$showData = "SELECT nv.id as id, ma_nv, hinh_anh, ten_nv, biet_danh, gioi_tinh, 
    nv.ngay_tao as ngay_tao, ngay_sinh, noi_sinh, so_cmnd, ten_tinh_trang, ngay_cap_cmnd, noi_cap_cmnd, 
    nguyen_quan, ten_quoc_tich, ten_dan_toc, ten_ton_giao, ho_khau, tam_tru, ten_loai_nv, 
    ten_trinh_do, ten_chuyen_mon, ten_bang_cap, ten_chuc_vu, trang_thai 
    FROM nhanvien nv, quoc_tich qt, dan_toc dt, ton_giao tg, loai_nv lnv, trinh_do td, chuyen_mon cm, bang_cap bc, chuc_vu cv, tinh_trang_hon_nhan hn 
    WHERE nv.quoc_tich_id = qt.id AND nv.dan_toc_id = dt.id AND nv.ton_giao_id = tg.id 
    AND nv.loai_nv_id = lnv.id AND nv.trinh_do_id = td.id AND nv.chuyen_mon_id = cm.id AND nv.bang_cap_id = bc.id 
     AND nv.chuc_vu_id = cv.id AND nv.hon_nhan_id = hn.id AND nv.id = $id";
		$result = mysqli_query($conn, $showData);
		$row = mysqli_fetch_array($result);

		$sqlPhongBan = "SELECT pb.* FROM phong_ban as pb INNER JOIN chi_tiet_phong_ban as ctpb ON pb.id = ctpb.phong_ban_id WHERE nhan_vien_id = $id";

		$resultPhongBan = mysqli_query($conn, $sqlPhongBan);

		$dataPhongBan = array();
		while ($rowPhongBan = mysqli_fetch_array($resultPhongBan)) {
			$dataPhongBan[] = $rowPhongBan;
		}
	}
							...
							<div class="row">
								...
								<div class="col-lg-5 col-sm-5 col-md-6 col-xs-12">
									...
									<p class="box-title">Ngày cấp:
										<?php $ngayCap = date_create($row['ngay_cap_cmnd']);
										echo date_format($ngayCap, 'd-m-Y'); ?>
									</p>
								...
								<div class="col-lg-5 col-sm-5 col-md-6 col-xs-12">
									...
									<p class="box-title">Phòng ban:
										<?php
										if (!empty($dataPhongBan)) {
											foreach ($dataPhongBan as $pb) {
												echo "<p><b>" . $pb['ten_phong_ban'] . "</b></p>";
											}
										}
										?>
									</p>
